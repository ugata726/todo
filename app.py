...
with st.form("task_form", clear_on_submit=True):
    ...
    if submitted:
        if title.strip() != "":
            c.execute(
                "INSERT INTO tasks (menu, title, importance, due_date) VALUES (?, ?, ?, ?)",
                (selected_menu, title, importance, due_date.isoformat())
            )
            conn.commit()
...
st.header("タスクリスト")
c.execute("""
SELECT id, menu, title, importance, due_date, done 
FROM tasks 
WHERE menu = ? 
ORDER BY 
    CASE importance 
...
if task_ids:
    selected_task_id = st.selectbox(
        "選択タスク", 
        task_ids, 
        format_func=lambda x: f"{x} | {next(r[2] for r in rows if r[0]==x)}"
    )
    task = next(r for r in rows if r[0]==selected_task_id)
    st.write(f"**タイトル:** {task[2]}")
    st.write(f"**メニュー:** {task[1]}")
    st.write(f"**重要度:** {task[3]}")
    st.write(f"**締切日:** {task[4]}")
    delete = st.button("削除")
    if delete:
